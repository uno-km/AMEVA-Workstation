...
export function DynamicRemotePluginLoader({ pluginId, scriptUrl }: { pluginId: string; scriptUrl: string }) {
  ...
  useEffect(() => {
    ...
    async function fetchAndLoad() {
      try {
        if (!scriptUrl) throw new Error("플러그인 주소(scriptUrl)가 제공되지 않았습니다.");
        
        const targetUrl = new URL(scriptUrl, window.location.origin);
        targetUrl.searchParams.set('t', Date.now().toString());
        
        const res = await fetch(targetUrl.toString());
        if (!res.ok) throw new Error("플러그인 다운로드에 실패했습니다. (HTTP " + res.status + ")");
