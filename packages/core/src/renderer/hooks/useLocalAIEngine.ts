...
export function useLocalAIEngine() {
  ...
  const loadModels = useCallback(async (type: 'chat' | 'code' = 'chat') => {
    ...
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
      try {
        ...
        const list = await adapter.ai.getModels();
        const mappedList = list.map(m => ({
          path: m,
          filename: m.split('/').pop() || m,
          name: m.split('/').pop() || m,
          size: 0
        }));
        if (type === 'chat') {
        ...
        console.error('[Mobile loadModels] 에러:', err);
      }
      return;
    }
  ...
  const startEngine = useCallback(async () => {
    // 1. 모바일 환경 처리
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
      addSensorLog('[System] 모바일 네이티브 AI 엔진 기동 준비 완료.');
      setIsAvailable(true);
      return;
    }
  ...
  const stopEngine = useCallback(async () => {
    ...
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
      try {
        addSensorLog('[System] 모바일 네이티브 AI 엔진 정지 요청...');
        const adapter = getPlatformAdapter();
        await adapter.ai.stopAI();
        setIsAvailable(false);
        addSensorLog('[System] 모바일 엔진이 정지되었습니다.');
      } catch (err: any) {
        addSensorLog(`[Error] 모바일 엔진 정지 오류: ${err.message}`);
      }
      return;
    }
