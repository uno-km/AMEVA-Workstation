...
files.forEach(file => {
  ...
  // Add getPlatformAdapter import if needed and if we are going to modify
  let needsImport = false;
  if (content.includes('ipc.isElectronEnv()') || content.includes('!ipc.isElectronEnv()')) {
    needsImport = true;
  }
  ...
  content = content.replace(/ipc\.isFreeMode\(\)/g, "(!getPlatformAdapter().settings.getPlanStatus())");
...
console.log(Total modified files: );