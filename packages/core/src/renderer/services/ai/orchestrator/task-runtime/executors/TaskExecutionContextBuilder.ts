...
2. [매우 중요] 기존 문서나 보고서를 이어서 작성할 때는 절대 'write_file'로 덮어쓰지 말고, 반드시 'append_file' 도구를 사용하세요. (write_file을 연속해서 쓰면 이전 작업 내역이 전부 날아갑니다!)
3. 'run_command' 도구를 웹 검색(인터넷 정보 찾기) 용도로 사용하지 마세요. 에이전트에게는 현재 브라우저 검색 도구가 없으므로 상상(Hallucination)해서 인터넷 검색 명령을 내리면 안 됩니다. 아는 지식 내에서 최선을 다해 작성하세요.
...
(만약 'write_file'이나 'append_file' 도구로 문서를 저장했다면, 최종 답변 어딘가에 반드시 [FILE_PATH: 저장한경로] 형식의 태그를 포함해야 합니다!)
Final Answer: [완료된 보고서 요약 및 작성 결과물 설명] [FILE_PATH: 산출물.md]
[DONE]
