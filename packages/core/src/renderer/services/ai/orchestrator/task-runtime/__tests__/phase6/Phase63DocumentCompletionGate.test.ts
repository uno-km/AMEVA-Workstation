...
describe('Phase63DocumentCompletionGate', () => {
  ...
  it('should fail if extraction provenance is SYNTHETIC_FIXTURE_EXECUTED', () => {
    const gate = new DocumentCompletionGate();
    const res = gate.check({
      ...
      reopenResult: { passed: true, issues: [], similarityScore: 1, criticalFactsPreserved: true, requiredSectionsPreserved: true, placeholdersDetected: false, extractionResult: { extractionExecutionProvenance: 'SYNTHETIC_FIXTURE_EXECUTED', success: true, contentLength: 100 } as any }
    });
    expect(res.passed).toBe(false);
    expect(res.reasons.some(r => r.includes('Extraction provenance was not valid for operational completion: SYNTHETIC_FIXTURE_EXECUTED'))).toBe(true);
  });
});
