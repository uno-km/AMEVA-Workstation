...
import { formatBytes } from "../../utils/aiFormatters"
  ...
  /*
   * [FUNCTION CONTRACT]
   * - 함수 명: `AIDownloadProgress`
   * - 역할: 인자 정보를 검수하고 비즈니스 계약 조건에 맞춰 최종 바인딩 결과물/바이너리 버퍼를 반환함.
   * - 예시: `AIDownloadProgress(...)` 호출 시 런타임 비동기/동기 연쇄 반응 유도.
   */
export function AIDownloadProgress({ downloadStatus, onCancel, onShowDetails }: any) {
      /*
       * [ALGORITHM BRANCH / DECISION]
       * - 조건 식: `!downloadStatus || !downloadStatus.status`
       * - 만족 시: 비즈니스 요구사항을 만족하여 대응 내부 분기 블록을 구동함.
       * - 불만족 시: 바이패스(Bypass)하여 하위 연산으로 폴백하거나 조건 스택을 탈출함.
       * - 예시: `if (!downloadStatus || !downloadStatus.status)` 만족 시 런타임 내포 연산 및 데이터 매핑 즉시 활성화.
       */
