const fs = require('fs');
let content = fs.readFileSync('src/renderer/hooks/ai/useAIAgentMode.ts', 'utf-8');

const replacement = `import { useCallback, useRef } from 'react'
...
fs.writeFileSync('src/renderer/hooks/ai/useAIAgentMode.ts', replacement, 'utf-8');
console.log("Updated useAIAgentMode.ts with separated services");
