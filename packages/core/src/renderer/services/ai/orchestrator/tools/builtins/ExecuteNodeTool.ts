...
export class ExecuteNodeTool extends BaseTool {
  ...
  public readonly description = 'Node.js(JavaScript) 코드를 샌드박스 환경(또는 로컬)에서 실행하고 그 결과를 반환합니다. 스크립트 실행, 로직 검증 등에 사용하세요.';
  
  public readonly parameters = {
    type: 'object' as const,
    properties: {
      code: { type: 'string', description: '실행할 Node.js(JavaScript) 코드 블록' }
    },
    required: ['code']
  };
  ...
  protected async executeCore(args: Record<string, unknown>, context?: ToolExecutionContext): Promise<ToolCallResult> {
    ...
    try {
      // Create temporary file using powershell and execute it
      const b64 = btoa(unescape(encodeURIComponent(code)));
      const psCmd = `
        $tmpPath = Join-Path $env:TEMP "agent_sandbox_$([Guid]::NewGuid().ToString()).js"
